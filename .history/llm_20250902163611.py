...
def get_ai_response(user_message):
    ...
    topic_chain = RunnableLambda(lambda x: first_chain)
    full_chain = {"topic": topic_chain, "question": lambda x: x["question"]} | RunnableLambda(
        route
    )
    print(full_chain.invoke({"question": user_message}, {config={"configurable": {"session_id": "abc123"}}}))
